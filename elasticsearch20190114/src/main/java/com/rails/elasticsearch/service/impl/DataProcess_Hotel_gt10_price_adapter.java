...
public class DataProcess_Hotel_gt10_price_adapter implements MQ2Elasticsearch {
	...
	public boolean mqData2Elasticsearch(List<MessageRequest> datas) {
		...
		logger.info(
				"DataProcess_Hotel_gt10_price_adapter需要导入的数据总条数为：" + datas.size() + "条,以" + bulkNum + "条为一个批次进行导入。");
		...
		for (int i = 0; i < datas.size(); i++) {
			...
			if (parseObject != null) {
				// 格式化时间类型
				parseObject = JSONUtils.strToDate("yyyy-MM-dd HH:mm:ss", parseObject, "laterBookingTime");
				parseObject = JSONUtils.strToDate("yyyy-MM-dd HH:mm:ss", parseObject, "starCancel");
				parseObject = JSONUtils.strToDate("yyyy-MM-dd HH:mm:ss", parseObject, "endCancel");
				...
				String priceId = parseObject.get("productPriceId") + "_" + parseObject.get("city");
				IndexRequestBuilder setSource = client.prepareIndex("hotel", "hotel", priceId)
				...
				if (count % bulkNum == 0) {
					...
					if (response.hasFailures()) {
						...
						log.error("DataProcess_Hotel_gt10_price_adapter批量导入失败的数据为：" + collect);
						logger.error("DataProcess_Hotel_gt10_price_adapter批量导入失败的数据为：" + collect);
					} else {
						logger.info("DataProcess_Hotel_gt10_price_adapter批量导入的数据总条数为：" + bulkNum * loopNum + ",状态为："
								+ response.status());
					}
				}
			}
		...
		if (response.hasFailures()) {
			...
			log.error("DataProcess_Hotel_gt10_price_adapter批量导入后剩余的数据导入失败的数据为：" + collect);
			logger.error("DataProcess_Hotel_gt10_price_adapter批量导入后剩余的数据导入失败的数据为：" + collect);
		} else {
			logger.info("DataProcess_Hotel_gt10_price_adapter批量导入后剩余的数据导入的总条数为：" + (count - bulkNum * loopNum) + ",状态为："
					+ response.status());
		}
		return false;
	}
