...
public class DataProcess_Hotel_gt10_room_type_adapter implements MQ2Elasticsearch {
	...
	public boolean mqData2Elasticsearch(List<MessageRequest> datas) {
		...
		logger.info("DataProcess_Hotel_gt10_room_type_adapter需要导入的数据总条数为：" + datas.size() + "条,以" + bulkNum
				+ "条为一个批次进行导入。");
		...
		for (int i = 0; i < datas.size(); i++) {
			...
			if (parseObject != null) {
				parseObject.put("roomFacilitys", JSONArray.parseArray(parseObject.getString("roomFacilitys")));

				String roomId = parseObject.get("roomTypeAdapterId") + "_" + parseObject.get("city");

				IndexRequestBuilder setSource = client.prepareIndex("room_type", "room_type", roomId)
				...
				if (count % bulkNum == 0) {
					...
					if (response.hasFailures()) {
						...
						log.error("DataProcess_Hotel_gt10_room_type_adapter批量导入失败的数据为：" + collect);
						logger.error("DataProcess_Hotel_gt10_room_type_adapter批量导入失败的数据为：" + collect);
					} else {
						logger.info("DataProcess_Hotel_gt10_room_type_adapter批量导入的数据总条数为：" + bulkNum * loopNum + ",状态为："
								+ response.status());
					}
				}
			}
		...
		if (response.hasFailures()) {
			...
			log.error("DataProcess_Hotel_gt10_room_type_adapter批量导入后剩余的数据导入失败的数据为：" + collect);
			logger.error("DataProcess_Hotel_gt10_room_type_adapter批量导入后剩余的数据导入失败的数据为：" + collect);
		} else {
			logger.info("DataProcess_Hotel_gt10_room_type_adapter批量导入后剩余的数据导入的总条数为：" + (count - bulkNum * loopNum)
					+ ",状态为：" + response.status());
		}
		return false;
	}
